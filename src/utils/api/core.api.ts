...
export interface APIResponse extends PaginationMeta {
  items: any[];
}
...
export class CoreApi {
  ...
  fetchByPage(page: number,per_page: number, previous_keys?: any) {
    let params: any = { page, per_page };
    if (previous_keys!=='{}') {
        params.previous_keys = JSON.stringify(previous_keys); 
    }
    return this.http.get(this._base_path,{params})
  }
  findAll() {
  ...
  static defaultResponse: APIResponse = {
    items: [],
    current_page: 0,
    has_more: false,
    per_page: 0,
